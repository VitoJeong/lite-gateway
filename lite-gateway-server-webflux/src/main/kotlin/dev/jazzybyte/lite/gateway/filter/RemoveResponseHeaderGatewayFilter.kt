...
/**
 * 응답 헤더를 제거하는 게이트웨이 필터.
 * 
 * 이 필터는 나가는 HTTP 응답에서 지정된 헤더를 제거한다.
 * Post-filter 로직으로 동작하여 프록시 응답 후에 헤더를 제거한다.
 * Reactive Chain 패턴을 적용하여 chain.filter() 완료 후 헤더를 제거한다.
 * 
 * @param name 제거할 헤더의 이름
 * @param order 필터 실행 순서 (기본값: LOWEST_PRECEDENCE - 900)
 */
class RemoveResponseHeaderGatewayFilter(
    private val name: String,
    private val order: Int = Ordered.LOWEST_PRECEDENCE - 900
    ...
    override fun filter(context: GatewayContext, chain: GatewayFilterChain): Mono<Void> {
        return try {
            // Post-filter 로직: chain.filter() 완료 후 응답 헤더 제거
            ...
            Mono.error(RuntimeException("Failed to remove response header '$name': ${e.message}", e))
        }
    }
